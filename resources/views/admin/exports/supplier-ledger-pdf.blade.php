<x-khata-pdf-layout title="Supplier Ledger Statement">
    <x-slot:meta>
        <div class="khata-meta">
            <strong>Supplier:</strong> {{ $supplier->name }} ({{ $supplier->code }})
            @if($supplier->phone) &nbsp;|&nbsp; <strong>Phone:</strong> {{ $supplier->phone }} @endif
            @if($supplier->city) &nbsp;|&nbsp; <strong>City:</strong> {{ $supplier->city }} @endif
            <br>
    ...
    <table class="ledger">
        ...
        <tbody>
            ...
            @empty
            <tr><td colspan="6" style="text-align:center; color:#777;">No transactions in this period.</td></tr>
            @endforelse
            <tr class="total-row">
    ...
    <p style="font-size:9px; color:#777; margin-top:6px;">Note: only credit-term purchases affect this balance - cash purchases settle instantly and are excluded here, matching the supplier's balance shown throughout the system.</p>
</x-khata-pdf-layout>
