        ...
        <div class="bg-white rounded-xl shadow-card overflow-hidden">
            ...
            <div class="p-6 space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">{{ $purchaseReturn->return_no }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">PO #</span>
                    <a href="{{ route('admin.purchases.show', $purchaseReturn->purchase) }}" class="text-blue-600 hover:underline">
                        {{ $purchaseReturn->purchase->invoice_no ?? 'N/A' }}
                    </a>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">Supplier</span>
                    <span class="font-medium">{{ $purchaseReturn->supplier->name ?? 'N/A' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">Return Date</span>
                    <span class="font-medium">{{ $purchaseReturn->return_date->format('d-m-Y') }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">Refund Method</span>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                        {{ $purchaseReturn->refund_method_label }}
                    </span>
                </div>
                @if($purchaseReturn->reason)
                <div class="mt-2 p-3 bg-gray-50 rounded-lg">
                    <p class="text-xs text-gray-500">Reason</p>
                    <p class="text-sm text-gray-900">{{ $purchaseReturn->reason }}</p>
                </div>
                @endif
                @if($purchaseReturn->notes)
                <div class="mt-2 p-3 bg-gray-50 rounded-lg">
                    <p class="text-xs text-gray-500">Notes</p>
                    <p class="text-sm text-gray-900">{{ $purchaseReturn->notes }}</p>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-card overflow-hidden">
            ...
            <div class="p-6 space-y-3">
                <div class="flex justify-between">
                    ...
                    <span class="font-medium">Rs. {{ number_format($purchaseReturn->sub_total, 2) }}</span>
                </div>
                @if($purchaseReturn->discount > 0)
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">Discount</span>
                    <span class="font-medium text-red-600">- Rs. {{ number_format($purchaseReturn->discount, 2) }}</span>
                </div>
                @endif
                @if($purchaseReturn->tax > 0)
                <div class="flex justify-between">
                    <span class="text-sm text-gray-500">Tax</span>
                    <span class="font-medium">Rs. {{ number_format($purchaseReturn->tax, 2) }}</span>
                </div>
                @endif
                <div class="flex justify-between border-t border-gray-200 pt-2">
                    <span class="text-sm font-bold text-gray-900">Total Return</span>
                    <span class="text-lg font-bold text-red-600">Rs. {{ number_format($purchaseReturn->total_amount, 2) }}</span>
                </div>
            </div>
        </div>

        <div class="flex gap-2">
            <a href="{{ route('admin.purchase-returns.index') }}" class="flex-1 px-4 py-2 bg-gray-200 text-gray-700 text-center rounded-lg font-medium hover:bg-gray-300 transition-colors duration-200">
                <i class="fas fa-arrow-left mr-1"></i> Back
            </a>
            <form action="{{ route('admin.purchase-returns.destroy', $purchaseReturn) }}" method="POST" class="flex-1">
                @csrf
                @method('DELETE')
                ...
                <table class="w-full">
                    ...
                    <tfoot class="bg-gray-50 font-medium">
                        <tr>
                            ...
                            <td class="text-right py-2 px-2">Rs. {{ number_format($purchaseReturn->sub_total, 2) }}</td>
                        </tr>
                        @if($purchaseReturn->discount > 0)
                        <tr>
                            <td colspan="5" class="text-right py-2 px-2">Discount:</td>
                            <td class="text-right py-2 px-2 text-red-600">- Rs. {{ number_format($purchaseReturn->discount, 2) }}</td>
                        </tr>
                        @endif
                        @if($purchaseReturn->tax > 0)
                        <tr>
                            <td colspan="5" class="text-right py-2 px-2">Tax:</td>
                            <td class="text-right py-2 px-2">Rs. {{ number_format($purchaseReturn->tax, 2) }}</td>
                        </tr>
                        @endif
                        <tr class="text-lg">
                            <td colspan="5" class="text-right py-2 px-2 font-bold">Total Return:</td>
                            <td class="text-right py-2 px-2 font-bold text-red-600">Rs. {{ number_format($purchaseReturn->total_amount, 2) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
