                            ...
                            <tbody>
                                <template x-for="(item, index) in items" :key="index">
                                    <tr class="border-b border-gray-100 hover:bg-blue-50/30 transition-colors duration-150">
                                        ...
                                        <td class="py-1.5 px-1.5">
                                            <input type="number" step="0.01" :name="'items['+index+'][quantity]'" x-model="item.quantity" @input="calculateRow(index)" class="w-full px-1 py-1 text-sm text-center border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" min="0.01" step="0.01">
                                        </td>
                                        <td class="py-1.5 px-1.5">
                                            <input type="number" step="0.01" :name="'items['+index+'][unit_price]'" x-model="item.unit_price" @input="calculateRow(index)" class="w-full px-1 py-1 text-sm text-right border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" min="0" step="0.01">
                                        </td>
                                        <td class="py-1.5 px-1.5">
                    ...
                    <div class="sm:hidden space-y-2">
                        <template x-for="(item, index) in items" :key="index">
                            <div class="bg-gray-50 border border-gray-200 rounded-lg p-3 space-y-2">
                                <select :name="'items['+index+'][product_id]'" x-model="item.product_id" @change="calculateRow(index)" class="w-full px-2 py-1.5 text-sm border border-gray-300 rounded-lg bg-white">
                                    <option value="">Select Product</option>
                                    @foreach($products as $product)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex items-center gap-2 justify-end">
                                ...
                                <select name="discount_type" class="w-16 px-1 py-1 text-xs border border-gray-300 rounded-lg">
                                    <option value="fixed" {{ $sale->discount_type == 'fixed' ? 'selected' : '' }}>Rs</option>
                                    <option value="percentage" {{ $sale->discount_type == 'percentage' ? 'selected' : '' }}>%</option>
                                </select>
...
<script>
// =============================================
// ✅ FIXED: Using stringified JSON to avoid Blade parsing
// =============================================

function saleForm() {
    // Pass data as pre-encoded JSON strings to avoid Blade parsing issues
    var agents = JSON.parse('{!! json_encode($agents) !!}');
    var existingItems = JSON.parse('{!! json_encode($sale->items->map(function($item) {
        return [
    ...
    return {
        ...
        updateCommission: function() {
            ...
            var agent = this.agents.find(function(a) {
                return a.id == self.agent_id;
            });
            
            if (agent) {
                this.commissionRate = parseFloat(agent.commission_rate_cash) || 0;
            } else {
        ...
        calculateTotals: function() {
            ...
            var discountType = document.querySelector('[name="discount_type"]')?.value || 'fixed';
            ...
            if (this.agent_id && this.commissionRate > 0) {
                this.commissionAmount = netTotal * this.commissionRate / 100;
            } else {
