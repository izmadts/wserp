...
function salesReturnForm() {
    return {
        ...
        // the actual return line data (which sale_item_id it reverses, its
        // original price/discount/tax) ever got copied over, so every row
        // silently submitted an empty sale_item_id and Rs. 0 price. Since
        // items.*.sale_item_id is required server-side, the form failed
        // validation on every single submission - and with no $errors
        // display on this page (added above), that looked exactly like
        // "nothing happens" when you click Create Return.
        onProductChange(index, event) {
            const option = event.target.selectedOptions[0];
            const item = this.items[index];

            if (option && option.value) {
                item.sale_item_id = option.dataset.saleItemId;
