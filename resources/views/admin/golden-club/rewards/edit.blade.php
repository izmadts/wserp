...
<div class="bg-white rounded-xl shadow-card overflow-hidden" x-data="{
        rewardType: '{{ old('reward_type', $reward->reward_type) }}',
        productId: '{{ old('product_id', $reward->product_id) }}',
        isProductType() { return this.rewardType === 'gift' || this.rewardType === 'product'; },
    }">
    <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
        <h3 class="text-base sm:text-lg font-semibold text-gray-900">
            <i class="fas fa-edit text-yellow-600 mr-2"></i> Edit Reward: {{ $reward->title }}
        </h3>
    </div>

    <div class="p-4 sm:p-6">
        <form action="{{ route('admin.golden-club.rewards.update', $reward) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $reward->title) }}" required
                ...
                <div>
                    ...
                    <select name="reward_type" x-model="rewardType" required
                        ...
                        <option value="gift" {{ old('reward_type', $reward->reward_type) == 'gift' ? 'selected' : '' }}>Gift</option>
                        <option value="product" {{ old('reward_type', $reward->reward_type) == 'product' ? 'selected' : '' }}>Product</option>
                        <option value="coupon" {{ old('reward_type', $reward->reward_type) == 'coupon' ? 'selected' : '' }}>Coupon</option>
                        <option value="discount" {{ old('reward_type', $reward->reward_type) == 'discount' ? 'selected' : '' }}>Discount</option>
                        <option value="free_delivery" {{ old('reward_type', $reward->reward_type) == 'free_delivery' ? 'selected' : '' }}>Free Delivery</option>
                        <option value="lucky_draw_entry" {{ old('reward_type', $reward->reward_type) == 'lucky_draw_entry' ? 'selected' : '' }}>Lucky Draw Entry</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="points_required" value="{{ old('points_required', $reward->points_required) }}" required
                        ...
                        @foreach(['silver' => 'Silver', 'gold' => 'Gold', 'platinum' => 'Platinum'] as $value => $label)
                            <label class="inline-flex items-center gap-1.5 text-sm text-gray-700">
                                <input type="checkbox" name="eligible_membership_levels[]" value="{{ $value }}"
                                    {{ in_array($value, (array) old('eligible_membership_levels', $reward->eligible_membership_levels ?? [])) ? 'checked' : '' }}
                ...
                <div x-show="isProductType()" x-cloak class="md:col-span-2 bg-blue-50 border border-blue-100 rounded-lg p-3">
                    ...
                    <select name="product_id" x-model="productId"
                        ...
                        @foreach($loyaltyProducts as $product)
                            <option value="{{ $product->id }}" {{ old('product_id', $reward->product_id) == $product->id ? 'selected' : '' }}>{{ $product->name }} ({{ number_format($product->current_stock, 0) }} in stock)</option>
                        @endforeach
                    </select>
                ...
                <div x-show="!(isProductType() && productId)" x-cloak>
                    ...
                    <input type="number" min="0" name="stock" value="{{ old('stock', $reward->stock) }}"
                ...
                <template x-if="rewardType === 'coupon' || rewardType === 'discount'">
                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 bg-gray-50 border border-gray-200 rounded-lg p-3">
                        <div>
                            ...
                            <input type="number" step="0.01" min="0" name="metadata_discount_amount" value="{{ old('metadata_discount_amount', $reward->metadata['discount_amount'] ?? null) }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">or Discount %</label>
                            <input type="number" step="0.01" min="0" max="100" name="metadata_discount_percent" value="{{ old('metadata_discount_percent', $reward->metadata['discount_percent'] ?? null) }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Coupon Code</label>
                            <input type="text" name="metadata_coupon_code" value="{{ old('metadata_coupon_code', $reward->metadata['coupon_code'] ?? null) }}" placeholder="Leave blank to auto-generate"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                </template>
                ...
                <template x-if="rewardType === 'lucky_draw_entry'">
                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 border border-gray-200 rounded-lg p-3">
                        <div>
                            ...
                            <input type="number" min="1" name="metadata_entry_count" value="{{ old('metadata_entry_count', $reward->metadata['entry_count'] ?? 1) }}"
                        ...
                        <div>
                            ...
                            <select name="metadata_campaign_id"
                                ...
                                @foreach($campaigns as $campaign)
                                    <option value="{{ $campaign->id }}" {{ old('metadata_campaign_id', $reward->metadata['campaign_id'] ?? null) == $campaign->id ? 'selected' : '' }}>{{ $campaign->title }} ({{ ucfirst($campaign->status) }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    @if($reward->image)
                    <img src="{{ Storage::url($reward->image) }}" alt="{{ $reward->title }}" class="h-16 w-16 object-cover rounded-lg mb-2">
                    @endif
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $reward->description) }}</textarea>
            ...
            <div class="mt-6 flex flex-wrap items-center gap-3">
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors duration-200">
                    <i class="fas fa-save mr-1"></i> Update Reward
                </button>
